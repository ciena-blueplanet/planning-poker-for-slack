    ...
    .end(function (err, res) {
      ...
      .end(function (err, res) {
        ...
        .end(function (err, res) {
          if (err) return done(err)
          should.equal(res.status, 200, 'Status code for stop is not 200')
          expect(res.body.text).to.contain('Planning for JIRA-' + jiraId + ' is complete.')
          expect(res.body.text).to.contain('Average vote : 8.00')
          done()
        })
      })
    })
    ...
    .end(function (err, res) {
      ...
      .end(function (err, res) {
        ...
        .end(function (err, res) {
          if (err) return done(err)
          should.equal(res.status, 200, 'Status code for stop is not 200')
          expect(res.body.text).to.contain('Planning for JIRA-' + jiraId)
          expect(res.body.text).to.contain('Following members have abstained from voting : \nuser')
          done()
        })
      })
    })
